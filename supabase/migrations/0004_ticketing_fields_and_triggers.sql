...
create or replace function set_conversation_resolved_at()
returns trigger as $$
begin
...
drop trigger if exists conversations_set_resolved_at on conversations;
create trigger conversations_set_resolved_at
  before update on conversations
  for each row
  execute function set_conversation_resolved_at();
