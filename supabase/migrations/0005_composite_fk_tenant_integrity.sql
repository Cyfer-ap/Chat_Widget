...
alter table visitors
  add constraint visitors_id_tenant_id_key
  unique (id, tenant_id);

alter table conversations
  add constraint conversations_id_tenant_id_key
  unique (id, tenant_id);

-- Step 2: conversations → visitors composite FK
alter table conversations
  drop constraint if exists conversations_visitor_id_fkey;

alter table conversations
  add constraint conversations_visitor_id_tenant_id_fkey
    foreign key (visitor_id, tenant_id)
    references visitors (id, tenant_id)
    on delete cascade;
