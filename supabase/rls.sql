...
alter table public.bootstrap_log enable row level security;
alter table public.agent_invites enable row level security;

-- Drop existing policies (safe to re-run)
drop policy if exists "Agents can read tenant data" on public.tenants;
drop policy if exists "Agents can read sites" on public.tenant_sites;

drop policy if exists "Agents can read agents" on public.agents;
drop policy if exists "Agents can read their agent row" on public.agents;

drop policy if exists "Agents can read visitors" on public.visitors;
drop policy if exists "Agents can read conversations" on public.conversations;
drop policy if exists "Agents can update conversations" on public.conversations;
drop policy if exists "Agents can read messages" on public.messages;
drop policy if exists "Agents can insert messages" on public.messages;

drop policy if exists "Visitors can read their visitor record" on public.visitors;
drop policy if exists "Visitors can insert their visitor record" on public.visitors;
drop policy if exists "Visitors can insert conversations" on public.conversations;
drop policy if exists "Visitors can read their conversations" on public.conversations;
drop policy if exists "Visitors can read their messages" on public.messages;
...
create policy "Agents can read sites"
on public.tenant_sites
for select
               using (
               exists (
               select 1
               from public.agents a
               where a.tenant_id = tenant_sites.tenant_id
               and a.user_id = auth.uid()
               )
               );
...
create policy "Agents can read their agent row"
on public.agents
for select
...
create policy "Agents can read visitors"
on public.visitors
for select
               using (
               exists (
               select 1
               from public.agents a
               where a.tenant_id = visitors.tenant_id
               and a.user_id = auth.uid()
               )
               );
...
for select
               ...
               select 1
               from public.agents a
               where a.tenant_id = conversations.tenant_id
               and a.user_id = auth.uid()
               )
               );
...
create policy "Visitors can insert their visitor record"
on public.visitors
for insert
with check (visitors.anon_id = auth.uid()::text);
