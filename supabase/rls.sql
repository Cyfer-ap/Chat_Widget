...
drop policy if exists "Agents can read tenant data" on tenants;
drop policy if exists "Agents can read sites" on tenant_sites;
drop policy if exists "Agents can read visitors" on visitors;
drop policy if exists "Agents can read conversations" on conversations;
drop policy if exists "Agents can update conversations" on conversations;
drop policy if exists "Agents can read messages" on messages;
drop policy if exists "Agents can insert messages" on messages;
drop policy if exists "Visitors can read their visitor record" on visitors;
drop policy if exists "Visitors can insert their visitor record" on visitors;
...
create policy "Agents can read tenant data"
  ...
  using (exists (
    ...
    where agents.tenant_id = tenants.id
...
create policy "Agents can read sites"
  ...
  using (exists (
    select 1 from agents
    where agents.tenant_id = tenant_sites.tenant_id
      and agents.user_id = auth.uid()
  ));
...
create policy "Agents can read visitors"
  ...
  using (exists (
    select 1 from agents
    where agents.tenant_id = visitors.tenant_id
      and agents.user_id = auth.uid()
  ));
