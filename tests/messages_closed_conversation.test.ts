...
if (!canRunIntegration) {
  ...
  test("DB trigger blocks inserts into closed conversations and allows when open", async () => {
    ...
    const tRes = await service.from("tenants").insert({ name: "test-tenant-closed" }).select().throwOnError();
    const tenant = Array.isArray(tRes.data) ? tRes.data[0] : tRes.data;

    const vRes = await service.from("visitors").insert({ tenant_id: tenant.id, anon_id: "test-anon-closed" }).select().throwOnError();
    const visitor = Array.isArray(vRes.data) ? vRes.data[0] : vRes.data;

    const cRes = await service.from("conversations").insert({ tenant_id: tenant.id, visitor_id: visitor.id }).select().throwOnError();
    ...
    assert.ok(!msgOpen.error, `expected insert while open to succeed but got error: ${JSON.stringify(msgOpen.error)}`);

    // Close the conversation
    const upd = await service.from("conversations").update({ status: "closed" }).eq("id", conversation.id).select().throwOnError();
    ...
    assert.ok(msgClosed.error, `expected insert into closed conversation to be rejected but it succeeded: ${JSON.stringify(msgClosed.data)}`);
