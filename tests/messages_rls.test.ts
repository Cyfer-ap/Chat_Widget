...
if (!canRunIntegration) {
  ...
  test("anonymous insert into messages is rejected by RLS", async () => {
    ...
    const tRes = await service.from("tenants").insert({ name: "test-tenant" }).select().throwOnError();
    const tenant = Array.isArray(tRes.data) ? tRes.data[0] : tRes.data;
    assert.ok(tenant && tenant.id, "tenant created");

    const vRes = await service.from("visitors").insert({ tenant_id: tenant.id, anon_id: "test-anon-uid" }).select().throwOnError();
    ...
    assert.ok(insertRes.error, `Expected insert to be rejected by RLS but it succeeded: ${JSON.stringify(insertRes.data)}`);
